...
for i, column in enumerate(columns):
    plt.plot(
        df.date.values, df[column].values, lw=1.5, color=mycolors[i]
    )
    plt.text(
        df.shape[0] + 1,
        df[column].values[-1],
...
plt.title('Number of Deaths from Lung Diseases in the UK (1974-1979)', fontsize=18)
plt.yticks(
...
plt.ylim(y_LL, y_UL)
plt.xlim(-2, 80)
plt.savefig("../photos/change/multipleTimeSeriesPlot.png")
plt.show()
